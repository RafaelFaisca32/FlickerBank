...
public class CreateClientController implements Initializable {
    ...
    private String payFlickrAddress;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        pAddress_box.selectedProperty().addListener((observableValue, oldVal, newVal) -> {
            if (newVal){
                payFlickrAddress = createPayFlickrAddress();
    ...
    private void createClient() {
        ...
        Model.getInstance().getDatabaseDriver().createClient(fName, lName, payFlickrAddress, password, LocalDate.now());
    ...
    private void createAccount(String accountType) {
        ...
        if (accountType.equals("Checking")) {
            Model.getInstance().getDatabaseDriver().createCheckingAccount(payFlickrAddress, accountNumber, 10, balance);
        } else {
            Model.getInstance().getDatabaseDriver().createSavingsAccount(payFlickrAddress, accountNumber, 2000, balance);
        }
    }

    private void onCreatePayeeAddress() {
        if (fName_fld.getText() != null & lName_fld.getText() != null){
            pAddress_lbl.setText(payFlickrAddress);
        }
    }

    private String createPayFlickrAddress() {
